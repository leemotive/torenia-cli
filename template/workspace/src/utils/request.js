...
axios.interceptors.response.use((response) => {
  ...
  const { msg } = data;
  if (!isSuccess(data)) {
    errMsg === false || notification.error({
      message: '请求错误',
      description: msg || errMsg || `${methodMeaning[method]}失败` || '请求失败',
    });
  } else {
    (successMsg === false || !successMsg && method === 'get' && !msg) || message.success(msg || successMsg || `${methodMeaning[method]}成功` || '请求成功');
  }
